    ...
    if commonBackTo >= 0 then
        if deltalv > 0 then
            extendWithStrFmt(result, '* lists A and B are equal again from index %d for A, %d for B', len_a -
                commonBackTo, len_b - commonBackTo)
        else
...
function M.assertErrorMsgContains(partialMsg, func, ...)
    ...
    local no_error, error_msg = pcall(func, ...)
    if no_error then
        failure('No error generated when calling function but expected error containing: ' .. prettystr(partialMsg), nil,
            2)
    end
...
function JUnitOutput:endSuite()
    ...
    self.fd:write(string.format(
        ...
        self.result.runCount, self.result.startIsodate, self.result.duration, self.result.errorCount,
        self.result.failureCount, self.result.skippedCount))
        ...
        if M.LuaUnit.asFunction(instance) then
            ...
            if className then
                ...
                if methodInstance == nil then
                    error("Could not find method in class " .. tostring(className) ..
                        " for method " .. tostring(methodName))
                end
                table.insert(result, { name, instance })
            else
